...
import lugar.consultas as consultas_lugar
...
def crear_ventana_inicio(conn=None):
    ...
    data_values = list(consultas_lugar.cargar_tabla(conn))
...
def crear_ventana_actualizacion(dato:Local):
    ...
    layout = [
        [
            ...
            psg.Text(dato.lugar_id, visible=True, key='id-txt', expand_x=True),
            psg.Input(visible=False, key='id', default_text=dato.lugar_id)
        ],
        [
            psg.Text('Nombre', size=(ancho_etiqueta, 1)),
